...
impl Connection {
        ...
        SSt: Stream<Item = Result<ControlPacket<Clientbound>, Error>> + 'static + Send,
    {
        ...
        cert_builder.set_not_after(&Asn1Time::days_from_now(1).expect("Invalid time")).expect("Failed to set expiry");
        cert_builder.set_not_before(&Asn1Time::days_from_now(0).expect("Invalid time")).expect("Failed to set start time");
        cert_builder.set_pubkey(&key).expect("Failed to set pubkey");
        cert_builder.sign(&key, MessageDigest::sha256()).expect("Failed to sign certificate");
    ...
    fn setup_ice(&mut self) -> Result<(), Error> {
        ...
        let fingerprint = self.dtls_cert
        ...
        acceptor.set_certificate(&self.dtls_cert)
            .map_err(|e| Error::Protocol(format!("Failed to set certificate: {}", e)))?;
        acceptor.set_private_key(&self.dtls_key)
        ...
        match stream.poll_next(cx) {
            Poll::Ready(Some(mut candidate)) => {
                ...
                match (&mut candidate.address, self.config.ice_ipv4, self.config.ice_ipv6) {
                    (webrtc_sdp::address::Address::Ip(std::net::IpAddr::V4(addr)), Some(public), _) => {
                        *addr = public;
                    }
                    (webrtc_sdp::address::Address::Ip(std::net::IpAddr::V6(addr)), _, Some(public)) => {
                        *addr = public;
                    }
                    _ => {}
                }
    ...
    fn process_packet_from_client(&mut self, packet: ControlPacket<Serverbound>) -> Result<(), Error> {
        match packet {
            ControlPacket::Authenticate(mut message) => {
                debug!(webrtc = message.get_webrtc(), "Received Authenticate message");
            ...
            ControlPacket::WebRTC(mut message) => {
                ...
                if let Some((_, stream)) = &mut self.ice {
                    if let (Ok(ufrag), Ok(pwd)) = (
                        std::ffi::CString::new(message.take_ice_ufrag()),
                        std::ffi::CString::new(message.take_ice_pwd()),
                    ) {
